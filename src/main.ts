...
function gameView(): string {
  ...
  return `<main class="game-page"><header class="game-header"><div class="game-logo"><span>♛</span><strong>100</strong><small>SIMPLE NUMBERS. BIG REACTIONS.</small></div><div class="header-status"><span class="round-pill">ROUND ${state.round}</span><span class="turn-pill">${escapeHtml(active.name)}'s ${state.phase==='target'?'choice':'turn'}</span></div><div class="header-actions"><button data-action="rules" aria-label="Rules">?</button><button data-action="settings" aria-label="Settings">⚙</button><button data-action="menu" aria-label="Menu">☰</button></div></header>
    ...
    <footer class="hand-dock"><div class="dock-prompt"><span class="eyebrow">${mine?.kind==='human'?'YOUR TWO CARDS':'SPECTATOR'}</span><strong>${mine?.kind==='human'?`${escapeHtml(mine.name)}'s hand`:'Watch the table'}</strong><small>${myTurn ? state.phase === 'target' ? 'Choose a highlighted player.' : 'Drag or flick a card to the table, or select it below.' : `${escapeHtml(active.name)} is ${state.phase==='target'?'choosing a player':'playing'}…`}</small></div><div class="local-hand">${hand.length?hand.map(card=>cardElement(card,selectedCard===card.id,`hand-card ${myTurn ? '' : 'waiting-hand'}`)).join(''):`<div class="waiting-cards"><img src="${backImage}" alt="face-down card"><img src="${backImage}" alt="face-down card"></div>`}</div><div class="dock-controls"><button class="primary-button play-button" data-action="play-selected" ${!selectedCard||!myTurn||state.phase!=='playing'?'disabled':''}>PLAY CARD <span>➜</span></button><label class="mood-label">MOOD <select id="live-mood" ${mine?.kind!=='human'?'disabled':''}>${MOODS.map(m=>`<option ${mine?.mood===m?'selected':''}>${m}</option>`).join('')}</select></label></div></footer>
