...
The live game is a Three.js scene: eight fixed chairs, a layered wood-and-cloth table, a sculpted pot with still liquid, an extruded gold total, physical card meshes and stacks, candlelit props, and a layered nighttime city beyond the windows. All 52 faces and the back use the supplied high-resolution artwork. Each front adds a thick suit-colored printed frame, Luckiest Guy numbers in opposite playing-card corners, and a concise instruction band on ranks 7–10. A separate rounded border ring carries a restrained metallic sheen on ordinary cards and stronger prismatic foil on ranks 7–10; the illustrated center stays clear. Cards render independently of room bloom and ambient occlusion while keeping correct depth and movement. Thin paper edges and the stack sides follow the same rounded shape. Each pile shows its top three physical cards over a compact body for hidden layers; its height tracks the actual count. Soft contact shadows sit beneath both piles.
